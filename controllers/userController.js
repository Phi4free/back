...
module.exports.verMeuPerfilGet = async (request, response) => {
    let { _id, nome, email, role, listaLeitura } = request.user;
    response.status(200).send({ message: 200, data: { _id, nome, email, role, listaLeitura } });
};
...
module.exports.removerListaPut = async (request, response, next) => {
    const body = request.body;
    const user = request.user;

    try {
        const result = await dbUpdateRemoveFromUserList(user._id, body._id);
        response
            .status(result.status)
            .send({ message: result.message, data: result.updatedUser });
    } catch (error) {
